...
Ob manuell oder mit Helm – der Trident-Operator ist eine großartige Möglichkeit, die Installation zu vereinfachen und Trident-Ressourcen dynamisch zu verwalten. Sie können sogar link:../trident-install/kubernetes-customize-deploy.html["Individuelle Anpassung der Trident Implementierung"] unter Verwendung der Attribute in der  `TridentOrchestrator` Custom Resource (CR).
