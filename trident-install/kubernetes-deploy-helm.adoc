...
.Bevor Sie beginnen
Zusätzlich zu dem link:kubernetes-deploy.html#before-you-deploy["Voraussetzungen für die Implementierung"] benötigen Sie link:https://v3.helm.sh/["Helm Version 3"^].
...
----
.  `helm install`Geben Sie einen Namen für Ihre Bereitstellung an, wie im folgenden Beispiel gezeigt, wo `100.2410.0` sich die Version von Trident befindet, die Sie installieren.
+
[listing]
----
